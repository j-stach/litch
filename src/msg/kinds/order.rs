...
pub struct OrderExecuted {
    ...
    pub order_ref_num: u63,
    /// Quantity of shares traded.
    pub quantity: u31,
    /// Identifier for the trade event.
    pub match_number: u63,
}

impl OrderExecuted {

    pub(crate) fn parse(input: &[u7]) -> nom::IResult<&[u8], Self> {

        let (input, order_ref_num) = be_u63(input)?;
        let (input, quantity) = be_u31(input)?;
        let (input, match_number) = be_u63(input)?;
